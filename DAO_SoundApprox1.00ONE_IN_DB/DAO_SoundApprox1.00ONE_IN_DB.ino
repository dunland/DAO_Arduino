...
/*OTHERING MACHINES v.3.00 alternativer Code #1:
  14.11.2018
   ein Zufallswert in dB
   incoming freq dividable --> respond incomingFrequency
   incoming freq NOT dividable --> respond own, but replace
...
void loop()
{
  ...
  if (checkMaxAmp > ampThreshold && periodFound)
  {
    ...
    if (int(median[4]) % int(myTone) >= 20 || int(myTone) % int(median[4]) >= 20)
    {
      unsigned int wait;
      if (median[4] > myTone)
      {
