...
/*OTHERING MACHINES v.3.05 alternativer Code #1:
   7.12.2018
   check for millis() in interrupt --> bug! cannot emit sound properly anymore
   reversed + renamed negative/positive reaction
   myTone cannot be increased yet because it stays in "loop"...
...
int subsideAfterReaction = 30000;
int toneDuration = 600; //length of sound emission in milliseconds
unsigned long restart = 0;
unsigned long timeToRestart = 1200000; //time in ms til next program restart
...
void loop()
{
  ...
  if (checkMaxAmp > ampThreshold && periodFound)
  {
    ...
    //    make int
    //    subtract float - int
    //    decimal: if smaller than 0.3: match
    //    if > 0.3: negative
    //    || 1 - decimal < 0.3 = match
