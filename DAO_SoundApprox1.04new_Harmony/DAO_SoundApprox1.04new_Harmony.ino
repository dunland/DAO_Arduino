...
/*OTHERING MACHINES v.3.02 alternativer Code #1:
   4.12.2018
   timedStatement fixed (vorher: Wert aus Datenbank, jetzt: myTone)
   short vib bei non-dividable
   myTone++ per second
   lightRGB(Dauer), lightsOn and lightsOff nur toggle
...
//    || 1 - decimal < 0.3 = match
    ...
    {
      positiveReaction(median[4]); //waits & lights, then responds incoming
    } else
    {
      unsigned int wait;
      if (median[4] > myTone)
      ...
      negativeReaction(median[4], wait); //vibrates, waits as calculated, responds new (halfway diff)
    }
