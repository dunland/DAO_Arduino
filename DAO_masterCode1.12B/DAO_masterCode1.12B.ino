...
byte maxAmp = 0;
byte checkMaxAmp;
byte ampThreshold = 8;//raise if you have a very noisy signal
byte midpoint = 204;
...
void positiveReaction(int playTone) {
  ...
  while (millis() < now + 1000) { 
    Serial.println("wait another second");
  }
  digitalWrite(TRIGGER, HIGH);
  tone(SPEAK, playTone, 300);
  now = millis();
  while (millis() < now + 500) {
    Serial.println("wait 500ms, until tone is over");
  }
...
void negativeReaction(int playTone, unsigned long wait) {
  ...
  now = millis();
  while (millis() < now + 1000) {
    Serial.println("vibrate for 1 second");
